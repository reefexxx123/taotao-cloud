...
public class CustomRedisTokenStore implements TokenStore {
    ...
    public OAuth2Authentication readAuthentication(OAuth2AccessToken token) {
        ...
        //是否开启token续签
        boolean isRenew = securityProperties.getAuth().getRenew().getEnabled();
    ...
    private int getAccessTokenValiditySeconds(String clientId) {
        ...
        byte[] bytes;
        try {
            bytes = conn.get(serializeKey(SecurityConstant.CACHE_CLIENT_KEY + ":" + clientId));
        } finally {
            conn.close();
        }
        if (bytes != null) {
        ...
        //返回默认值
        return SecurityConstant.ACCESS_TOKEN_VALIDITY_SECONDS;
    }
